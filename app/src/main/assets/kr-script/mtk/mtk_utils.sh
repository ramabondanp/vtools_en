...
ultra_limit_set() {
  if [[ "$state" == "1" ]]; then
    echo 'Raising the thermal limit disables the default CPU 120C auto-shutdown' 1>&2
    echo 'This is extremely dangerous and may cause overheating or failure!' 1>&2
    echo 'You are responsible for any consequences!' 1>&2
    echo 'If you are not ready, exit within 15 seconds!' 1>&2
